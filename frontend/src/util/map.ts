...
export function assignColorsPaths(paths: Path[]): Map<string,string> {
...
export function assignColorsWays(ways: Way[]): Map<string,string> {
  const m = new Map<string,string>();
  ways.forEach((way, i) => {
    const hue = (i * 137.508) % 360;
...
export async function fetchPaths(start: { lat: number, lon: number }, goal: { lat: number, lon: number }, distance: number, amount: number) {
    try {
